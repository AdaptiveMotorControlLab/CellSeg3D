        ...
        for h in range(H):
            for w in range(W):
                ...
                if h == -1:
                    if w == -1:
                        cropped_img[
                            10 : images.shape[1] + 10, 10 : images.shape[2] + 10, 0
                ...
                if h == -1:
                    if w == -1:
                        merged_imgs[
                            ...
                            10 : original_image_shape[1] + 10,
                            10 : original_image_shape[2] + 10,
                            0,
                        ]
                    elif w == 0:
...
def get_nested_unet(input_shape=(128, 128, 1), num_classes=3, deep_supervision=False):
    with tf.device("/gpu:0"):
        ...
        model.compile(
            optimizer=RMSprop(lr=0.0001), loss=bce_dice_loss, metrics=[dice_coeff]
        )
...
def train_unet(
    X_train, Y_train, csv_path, model_path, input_shape=(128, 128, 1), num_classes=1
):
    Y_train = Y_train
    X_train = X_train
...
def make_mask_img(ori_img, mask_img):
    ...
    masked_img = cv2.addWeighted(
        mask_img_rgb, 0.5, cv2.cvtColor(ori_img + 0.75, cv2.COLOR_GRAY2BGR), 0.6, 0
    )
    return masked_img
